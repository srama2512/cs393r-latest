...
const float HALF_FIELD_X = FIELD_X/2.0; // 1000
...
const float PENALTY_CROSS_X = HALF_FIELD_X - 800;
...
const Point2D circleLocation = Point2D(-HALF_GRASS_X, 0);
...
const Point2D landmarkLocation[] = {
  Point2D(-HALF_GRASS_X, 0),  // center circle
  ...
  Point2D( GOAL_X, 0 ),                 // WO_OWN_GOAL
  Point2D( -HALF_GRASS_X, 0),                 // WO_OPP_GOAL

  Point2D( GOAL_X, GOAL_Y / 2 ),        // WO_OWN_LEFT_GOALPOST
  Point2D( -HALF_GRASS_X, -GOAL_Y / 2),       // WO_OPP_LEFT_GOALPOST

  Point2D( GOAL_X, -GOAL_Y / 2),         // WO_OWN_RIGHT_GOALPOST
  Point2D( -HALF_GRASS_X, GOAL_Y / 2)        // WO_OPP_RIGHT_GOALPOST
};


// TODO - need to define correct intersectionLocatations
// Line intersection locations
const Point2D intersectionLocation[] = {
  // L
  Point2D( -FIELD_X / 2 + PENALTY_X, PENALTY_Y / 2),  // 4 WO_OPP_PEN_RIGHT_L
  Point2D( -FIELD_X / 2 + PENALTY_X, -PENALTY_Y / 2), //   WO_OPP_PEN_LEFT_L
  Point2D( -FIELD_X / 2, FIELD_Y / 2),                // 6 WO_OPP_FIELD_RIGHT_L
  Point2D( -FIELD_X / 2, -FIELD_Y / 2),               // 7 WO_OPP_FIELD_LEFT_L
  Point2D( FIELD_X / 2, FIELD_Y / 2),                 // 0 WO_OWN_FIELD_LEFT_L
  Point2D( FIELD_X / 2, -FIELD_Y / 2),                //   WO_OWN_FIELD_RIGHT_L
  Point2D( FIELD_X / 2 - PENALTY_X, PENALTY_Y / 2),   // 2 WO_OWN_PEN_LEFT_L
  Point2D( FIELD_X / 2 - PENALTY_X, -PENALTY_Y / 2),  //   WO_OWN_PEN_RIGHT_L
