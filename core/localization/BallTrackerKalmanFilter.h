...
public:
	...
	void mean_prediction(Eigen::Matrix<double, A, 1> u_t) override {
		this->mu_t_bar = this->G_t * this->mu_t + this->B_t * u_t;
	}

	Eigen::Matrix<double, K, 1> mean_measure() override {
		return this->H_t * this->mu_t_bar;
	}

	void jac_prediction() override {
		return;
	}

	void jac_correction() override {
		return;
	}
};
