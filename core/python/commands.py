...
def setHeadPanTilt(pan=0, tilt=-21, time=2.0, isChange=False):
    ...
    setHeadPan(pan, time / 2.0, isChange)
    setHeadTilt(tilt, time / 2.0)


def setHeadTilt(tilt=-22, target_time=2.0, isChange=False):
    """Set head tilt (up-down)."""
    joint_commands.setHeadTilt(core.DEG_T_RAD * tilt, target_time * 1000, isChange)
